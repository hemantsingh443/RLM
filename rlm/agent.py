"""
RLM Agent - Main Orchestration Logic.

Implements the recursive language model agent loop that coordinates
between the LLM (via OpenRouter) and the code execution sandbox.
Supports both local Docker and remote HTTP sandboxes.
"""

import os
from typing import List, Dict, Optional, Any, Union

from .clients.openrouter import OpenRouterClient
from .clients.docker_sandbox import DockerSandbox
from .clients.remote_sandbox import RemoteSandbox
from .prompts import format_system_prompt
from .parser import (
    extract_first_code_block,
    detect_final_answer,
    truncate_output,
    format_execution_result
)


class RLMAgent:
    """
    Recursive Language Model Agent.
    
    Orchestrates the conversation between an LLM and a code execution
    sandbox to analyze documents or directories.
    """
    
    DEFAULT_MAX_TURNS = 15
    DEFAULT_TRUNCATION_LIMIT = 30000  # High limit to avoid retry loops
    
    def __init__(
        self,
        api_key: Optional[str] = None,
        model: str = "xiaomi/mimo-v2-flash:free",
        max_turns: int = DEFAULT_MAX_TURNS,
        truncation_limit: int = DEFAULT_TRUNCATION_LIMIT,
        verbose: bool = True,
        remote_url: Optional[str] = None,
        remote_api_key: Optional[str] = None
    ):
        """
        Initialize the RLM Agent.
        
        Args:
            api_key: OpenRouter API key
            model: Model to use for root agent
            max_turns: Maximum conversation turns
            truncation_limit: Max chars for execution output
            verbose: Whether to print progress
            remote_url: URL for remote sandbox server (None for local Docker)
            remote_api_key: API key for remote sandbox auth
        """
        self.api_key = api_key or os.environ.get("OPENROUTER_API_KEY")
        self.model = model
        self.max_turns = max_turns
        self.truncation_limit = truncation_limit
        self.verbose = verbose
        self.remote_url = remote_url
        self.remote_api_key = remote_api_key
        
        self.llm_client = OpenRouterClient(
            api_key=self.api_key,
            default_model=self.model
        )
        
        self.sandbox: Optional[Union[DockerSandbox, RemoteSandbox]] = None
        self.history: List[Dict[str, str]] = []
        self.turn_count = 0
        self.is_directory_mode = False
    
    def _log(self, message: str):
        """Print a log message if verbose mode is enabled."""
        if self.verbose:
            print(message)
    
    def _get_file_stats(self, file_path: str) -> Dict[str, Any]:
        """Get statistics about a context file."""
        with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
            content = f.read()
        
        return {
            'length': len(content),
            'words': len(content.split()),
            'lines': len(content.splitlines()),
            'path': file_path
        }
    
    def _count_files(self, directory: str) -> int:
        """Count files in a directory."""
        count = 0
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if not d.startswith('.')]
            count += len([f for f in files if not f.startswith('.')])
        return count
    
    def _run_loop(self, query: str, system_prompt: str) -> str:
        """Run the main agent loop."""
        self.history = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Query: {query}"}
        ]
        
        for turn in range(self.max_turns):
            self.turn_count = turn + 1
            self._log(f"\n--- Turn {self.turn_count}/{self.max_turns} ---")
            
            # Call root LLM
            response = self.llm_client.chat(self.history)
            self._log(f"Model response:\n{response[:500]}{'...' if len(response) > 500 else ''}")
            
            # Extract code block
            code_block = extract_first_code_block(response)
            
            # Check for final answer
            is_final, answer_type, answer_content = detect_final_answer(response)
            
            if code_block:
                self._log(f"Executing code:\n{code_block[:300]}{'...' if len(code_block) > 300 else ''}")
                
                result = self.sandbox.exec_code(code_block)
                formatted_result = format_execution_result(result)
                truncated_result = truncate_output(formatted_result, self.truncation_limit)
                
                self._log(f"Execution result ({len(formatted_result)} chars):\n{truncated_result[:500]}")
                
                if is_final:
                    self._log(f"\nFinal answer detected ({answer_type})")
                    if answer_type == 'FINAL_VAR':
                        value = self.sandbox.get_variable(answer_content)
                        if value is not None:
                            return str(value)
                        return f"Variable '{answer_content}' not found. Output:\n{truncated_result}"
                    return answer_content
                
                self.history.append({"role": "assistant", "content": response})
                self.history.append({"role": "user", "content": f"Execution Result:\n{truncated_result}"})
            
            elif is_final:
                self._log(f"\nFinal answer detected ({answer_type})")
                if answer_type == 'FINAL_VAR':
                    value = self.sandbox.get_variable(answer_content)
                    if value is not None:
                        return str(value)
                    return f"Error: Variable '{answer_content}' not found"
                return answer_content
            
            else:
                self._log("No code block found, treating as reasoning step")
                self.history.append({"role": "assistant", "content": response})
                self.history.append({
                    "role": "user",
                    "content": "Continue with your analysis. Execute code or provide the final answer using FINAL()."
                })
        
        self._log("\nMax turns reached without final answer")
        return f"Error: Maximum turns reached. Last response:\n{response}"
    
    def run(
        self,
        query: str,
        context_file: str,
        context_type: str = "text document"
    ) -> str:
        """
        Run the RLM agent on a single file (legacy mode).
        
        Args:
            query: User's question/task
            context_file: Path to the context file
            context_type: Description of the context type
        
        Returns:
            Final answer string
        """
        self._log(f"\n{'='*60}")
        self._log(f"RLM Agent Starting")
        self._log(f"Query: {query}")
        self._log(f"Context: {context_file}")
        self._log(f"{'='*60}\n")
        
        file_stats = self._get_file_stats(context_file)
        self._log(f"Context stats: {file_stats['length']} chars, {file_stats['words']} words")
        
        # Initialize sandbox
        if self.remote_url:
            self._log(f"Connecting to remote sandbox: {self.remote_url}")
            self.sandbox = RemoteSandbox(self.remote_url, self.remote_api_key)
            if not self.sandbox.start():
                return "Error: Failed to connect to remote sandbox"
        else:
            self._log("Starting Docker sandbox...")
            self.sandbox = DockerSandbox(context_file=context_file, api_key=self.api_key)
            dockerfile_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            if not self._image_exists():
                self._log("Building Docker image...")
                if not self.sandbox.build_image(dockerfile_dir):
                    return "Error: Failed to build Docker image"
            if not self.sandbox.start():
                return "Error: Failed to start sandbox"
        
        try:
            system_prompt = format_system_prompt(
                context_length=file_stats['length'],
                context_type=context_type,
                is_directory=False
            )
            return self._run_loop(query, system_prompt)
        finally:
            self._log("\nStopping sandbox...")
            if self.sandbox:
                self.sandbox.stop()
    
    def run_directory(
        self,
        query: str,
        directory: str,
        context_type: str = "codebase"
    ) -> str:
        """
        Run the RLM agent on a directory (new mode).
        
        Args:
            query: User's question/task
            directory: Path to the directory to analyze
            context_type: Description of the context type
        
        Returns:
            Final answer string
        """
        self._log(f"\n{'='*60}")
        self._log(f"RLM Agent Starting (Directory Mode)")
        self._log(f"Query: {query}")
        self._log(f"Directory: {directory}")
        self._log(f"{'='*60}\n")
        
        self.is_directory_mode = True
        file_count = self._count_files(directory)
        self._log(f"Found {file_count} files in directory")
        
        # For directory mode, we need the remote server
        if self.remote_url:
            self._log(f"Connecting to remote sandbox: {self.remote_url}")
            self.sandbox = RemoteSandbox(self.remote_url, self.remote_api_key)
            if not self.sandbox.start():
                return "Error: Failed to connect to remote sandbox"
            
            # Reindex to pick up mounted files
            indexed = self.sandbox.reindex()
            self._log(f"Remote server indexed {indexed} files")
            file_count = indexed
        else:
            # Local mode: mount directory to Docker
            self._log("Starting Docker sandbox with directory mount...")
            self.sandbox = DockerSandbox(context_file=directory, api_key=self.api_key)
            dockerfile_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            if not self._image_exists():
                self._log("Building Docker image...")
                if not self.sandbox.build_image(dockerfile_dir):
                    return "Error: Failed to build Docker image"
            if not self.sandbox.start():
                return "Error: Failed to start sandbox"
        
        try:
            system_prompt = format_system_prompt(
                file_count=file_count,
                context_type=context_type,
                is_directory=True
            )
            return self._run_loop(query, system_prompt)
        finally:
            self._log("\nStopping sandbox...")
            if self.sandbox:
                self.sandbox.stop()
    
    def _image_exists(self) -> bool:
        """Check if the Docker image already exists."""
        import subprocess
        try:
            result = subprocess.run(
                ["docker", "images", "-q", DockerSandbox.IMAGE_NAME],
                capture_output=True,
                text=True
            )
            return bool(result.stdout.strip())
        except Exception:
            return False
    
    def get_history(self) -> List[Dict[str, str]]:
        """Get the conversation history."""
        return self.history.copy()
    
    def get_turn_count(self) -> int:
        """Get the number of turns completed."""
        return self.turn_count
